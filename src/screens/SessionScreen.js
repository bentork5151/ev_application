...
export default function SessionScreen({ route, navigation }) {
      ...
      try {
        ...
        if (stationId) {
          try {
            ...
            console.log('SessionScreen: Station cafe fetch failed, falling back:', e?.message);
          }
        }
        ...
        if (cafeList.length === 0 && lat && lng) {
          try {
            ...
            console.log('SessionScreen: Backend nearby cafe fetch failed:', e?.message);
          }
          ...
          if (cafeList.length === 0) {
            try {
              ...
              console.log('SessionScreen: Google Places fetch failed:', e?.message);
            }
          }
        }
        ...
        if (cafeList.length === 0) {
          try {
            ...
            console.log('SessionScreen: Fallback getAllLocations failed:', e?.message);
          }
        }

        const getFallbackImage = (category) => {
          const cat = (category || '').toLowerCase();
          if (cat.includes('mall') || cat.includes('shopping')) {
            return 'https://images.unsplash.com/photo-1519501025264-65ba15a82390?w=400&q=60';
          } else if (cat.includes('restaurant') || cat.includes('food') || cat.includes('dine')) {
            return 'https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=400&q=60';
          }
          return 'https://images.unsplash.com/photo-1501339847302-ac426a4a7cbb?w=400&q=60';
        };

        const getCafeImage = (loc) => {
          const uri = loc?.googleMapImageUrl ||
                      loc?.google_map_image_url ||
                      loc?.photoUrl || 
                      loc?.imageUrl || 
                      loc?.image_url || 
                      loc?.photo_url || 
          ...
          return getFallbackImage(loc?.category || loc?.type);
        };
        ...
        const mappedLocations = cafeList.map((loc) => ({
          ...
          fallbackImage: getFallbackImage(loc?.category || loc?.type),
          imgError: false,
        }));
        setBackendCafes(mappedLocations);
      } catch (err) {
        console.warn('SessionScreen: Failed to fetch cafes for station:', err);
        setBackendCafes([]);
      } finally {
  ...
  // Helper to redirect to InvoiceScreen whenever session ends
  const navigateToInvoice = (sId, sessionObj, energyVal) => {
    const targetSessionId = sId || sessionObj?.sessionId || sessionObj?.id || route?.params?.resumeSessionId || route?.params?.sessionId;
    navigation.replace('Invoice', {
      sessionId: targetSessionId,
      sessionData: sessionObj || activeSessionRef.current || route?.params,
      finalEnergy: energyVal !== undefined && energyVal > 0 ? energyVal : liveEnergy,
      stationName: sessionObj?.stationName || activeSessionRef.current?.stationName || route?.params?.stationName,
      rate: sessionObj?.rate || activeSessionRef.current?.rate || route?.params?.rate,
      chargerType: sessionObj?.chargerType || activeSessionRef.current?.chargerType || route?.params?.chargerType,
    });
  };
  ...
  useEffect(() => {
      ...
      try {
        ...
        if (userId) {
          ...
          if (session) {
            ...
            const passedId = route?.params?.resumeSessionId || route?.params?.sessionId;
            if (passedId) {
              navigateToInvoice(passedId, null, 0);
            }
          }
        }
      } catch (err) {
        console.warn('SessionScreen: Error fetching active session:', err);
      } finally {
    ...
    const fetchTelemetry = async (sId) => {
      ...
      try {
        ...
        if (currentStatus === 'COMPLETED' || currentStatus === 'STOPPED' || currentStatus === 'FINISHED' || currentStatus === 'INACTIVE') {
          navigateToInvoice(sessionId, activeSessionRef.current, energy || liveEnergy);
        }
      } catch (e) {
  ...
  const handleStopSession = async () => {
    const sId = activeSession?.sessionId || activeSession?.id || route?.params?.resumeSessionId || route?.params?.sessionId;
    if (!sId) {
    ...
    try {
      ...
      navigateToInvoice(sId, activeSession, liveEnergy);
    } catch (error) {
      console.error('SessionScreen: Failed to stop session:', error);
      ...
      <SafeAreaView style={styles.safeArea} edges={['bottom', 'left', 'right']}>
        <View style={styles.container}>
          ...
          <View style={styles.contentBody}>
            ...
            {(isLoadingCafes || backendCafes.length > 0) && (
                            ...
                            <Image
                              ...
                              onError={() => {
                                setBackendCafes((prev) =>
                                  prev.map((c, i) => (i === idx ? { ...c, image: c.fallbackImage, imgError: true } : c))
                                );
                              }}
                            />
                          ) : item.fallbackImage ? (
